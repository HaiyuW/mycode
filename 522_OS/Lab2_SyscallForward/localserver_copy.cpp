...
int main (int argc, char *argv[]) {
	...
	while (1) {
		...
		if (strcmp(msg,"ipc_close")==0) {
			struct redzone *s = (struct redzone *)redzone_start;
			int close_fd=s->fd;
			int close_ret_value = close(close_fd);
			...
			struct redzone *s = (struct redzone *)redzone_start;
			char open_filename[BUFSIZE];
			strcpy(open_filename,s->filename);
			int open_flags=s->flags;
			int open_mode=s->mode;
			int fd = open(open_filename,open_flags,open_mode);
			if (fd<0)
				handle_error("open");
			char ret[BUFSIZE];
			sprintf(ret, "%d", fd);
			...
			int read_fd=s->fd;
			int read_size=s->read_size;
						
			ssize_t count=read(read_fd,read_buf,read_size);
			if (count<0)
				handle_error("read");
			strncpy(s->str_mat[s->index],buf,read_size);
			// write(STDOUT_FILENO,s->str_mat[s->index],64);
			...
			struct redzone *s = (struct redzone *)redzone_start;
			int write_fd=s->fd;
			char write_buf[DATA_BUF_SIZE];
			int write_size=s->read_size;
			ssize_t count=write(write_fd, write_buf, write_size);
			if (count<0)
				handle_error("write");
			strncpy(s->str_mat[s->index],buf,write_size);
			char ret[BUFSIZE];
			sprintf(ret, "%d", count);
			if (send(clientfd, ret, sizeof(ret),0)<0)
				handle_error("send");
