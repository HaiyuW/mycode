...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    # get the ghost position with the largest belief
    ghostPostion = []
    # print livingGhostPositionDistributions[0]
    for beliefs in livingGhostPositionDistributions:
        maxProb = max(beliefs.values())
        for key, value in beliefs.items():
            if (value == maxProb):
                ghostPostion.append(key)
                break
    # get the ghost position with the closest distance
    ghostPosDict = {}
    for ghostPos in ghostPostion:
        ghostPosDict[ghostPos] = self.distancer.getDistance(pacmanPosition, ghostPos)
    # print ghostPosDict
    closestGhostPos = min(ghostPosDict, key=ghostPosDict.get)

    # form a dictionary containing each action and the successor distance
    nextStateDict = {}
    for action in legal:
        successorPosition = Actions.getSuccessor(pacmanPosition, action)
        nextStateDict[action] = self.distancer.getDistance(successorPosition, closestGhostPos)

    # choose the shortest distance action
    return min(nextStateDict, key=nextStateDict.get)
