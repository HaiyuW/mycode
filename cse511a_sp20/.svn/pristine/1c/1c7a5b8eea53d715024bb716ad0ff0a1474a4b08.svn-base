...
class ExactInference(InferenceModule):
  ...
  """
    ...
    """
        ...
        # handle jail. When notifyDistance is None, put ghost into jail
        # and set the self.beliefs to 0
        ...
        for p in self.legalPositions:
            ...
            # formula: B(p)=B(p)*P(noisyDistance | TrueDistance)
    ...
    """
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                # formula: B(newPos,t+1) = B(p,t) * P(newPos,t+1 | p,t) => B(Xt+1) = sum( B(Xt) * P(X'|Xt) )
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            allPossible[p] += currentBelief[p] * emissionModel[trueDistance]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numParticles):
            ...
            for j in range(self.numGhosts):
                particles.append(random.choice(self.legalPositions))
    ...
    def observeState(self, gameState):
        ...
        # for i, part in range(self.numParticles):
        #     for j in range(self.numGhosts):
        #         if noisyDistances[j] is None:
        #             part = list(self.particles[i])
        #             part[j] = self.getJailPosition(j)
        #             self.particles[i] = tuple(part)
        #             return
        ...
        for i, part in enumerate(self.particles):
            ...
            for j in range(self.numGhosts):
                if noisyDistances[j] is None:
                    ...
                    trueDistance = util.manhattanDistance(part[j], pacmanPosition)
                    prob = prob * emissionModels[j][trueDistance]
            allPossible[part] += prob

        # print self.particles[0]
        # print emissionModels[0]
        # print allPossible[part]
